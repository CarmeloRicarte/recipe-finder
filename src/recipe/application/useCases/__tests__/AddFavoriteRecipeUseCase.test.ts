...
describe("AddFavoriteRecipeUseCase", () => {
	it("should add a recipe to favorites", async () => {
		...
		const addFavoriteRecipeUseCase = new AddFavoriteRecipeUseCase(mockFavoriteRecipeRepository);

		const recipe: Recipe = mockRecipe;

		await addFavoriteRecipeUseCase.execute(recipe);

		expect(mockFavoriteRecipeRepository.addFavoriteRecipe).toHaveBeenCalledWith(recipe);
	});
	...
	it("should throw an error if the repository throws an error", async () => {
		...
		await expect(addFavoriteRecipeUseCase.execute(recipe)).rejects.toThrow("Repository error");
	});
});
