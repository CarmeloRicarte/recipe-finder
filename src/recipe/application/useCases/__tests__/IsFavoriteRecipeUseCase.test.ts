...
describe("IsFavoriteRecipeUseCase", () => {
	it("should return true if recipe is a favorite", async () => {
		...
		expect(mockFavoriteRecipeRepository.isFavoriteRecipe).toHaveBeenCalledWith("recipe1");
	});
	...
	it("should return false if recipe is not a favorite", async () => {
		...
		expect(mockFavoriteRecipeRepository.isFavoriteRecipe).toHaveBeenCalledWith("recipe2");
	});
	...
	it("should handle repository errors", async () => {
		...
		expect(mockFavoriteRecipeRepository.isFavoriteRecipe).toHaveBeenCalledWith("recipe3");
	});
});
